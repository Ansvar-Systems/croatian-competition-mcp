# ─────────────────────────────────────────────────────────────────────────────
# croatian-competition-mcp — multi-stage Dockerfile
# ─────────────────────────────────────────────────────────────────────────────
# Build:  docker build -t croatian-competition-mcp .
# Run:    docker run --rm -p 3000:3000 croatian-competition-mcp
#
# The image bakes /app/data/aztn.db at build time. Override AZTN_DB_PATH
...
ENV AZTN_DB_PATH=/app/data/aztn.db
...
# Bake the pre-built database into the image so /app/data/aztn.db resolves
# at runtime without a bind mount. This matches the working pattern in
# cypriot-data-protection-mcp and the Phase A canary fix from the
# 2026-05-10 sector MCP binding recovery.
#
...
# We then COPY it into the image at /app/data/aztn.db (AZTN_DB_PATH). The
# explicit `data/<name>.db` reference is required for the workflow's grep
# `COPY\s+\K(data/\S+\.db)` to match.
COPY data/database.db data/aztn.db
